...
public final class PaymentRepository {
    ...
    ///
    /// - Parameters:
    ///   - bodyData: The pre-built, ordered JSON request payload — see
    ///     ``PaymentRequest/addCardPayloadData(for:)``. Must be byte-exact ordered
    ///     JSON (not `.sortedKeys`-encoded), matching the same signature requirement
    ///     as ``submitPayment(request:card:)``.
    /// - Returns: The decoded ``PaymentResponse`` for the add-card operation.
    /// - Throws: ``PayOrcError`` on failure.
    public func addCard(bodyData: Data) async throws -> PaymentResponse {
        let endpoint = APIEndpoint.addCard(bodyData: bodyData)
        return try await networkClient.request(endpoint, responseType: PaymentResponse.self)
    }

    /// Completion-handler overload of ``addCard(bodyData:)``.
    public func addCard(
        bodyData: Data,
        completion: @escaping (Result<PaymentResponse, PayOrcError>) -> Void
    ) {
        Task {
            do {
                let result = try await addCard(bodyData: bodyData)
                await MainActor.run { completion(.success(result)) }
            } catch let error as PayOrcError {
                await MainActor.run { completion(.failure(error)) }
            } catch {
    ...
    /// Fetches saved cards for a customer/order and decodes them into ``CardData``.
    ///
    /// Mirrors Flutter's `CardRepo.listCustomerCards` + `CustomerCardsResponse.fromJson`.
    /// Uses ``NetworkClient/requestRawJSON(_:)`` because `data` here is an array of
    /// rows (dynamic keys), not the single-object shape ``PaymentResponse`` expects.
    public func listCustomerCards(mCustomerId: String, orderId: String) async throws -> [CardData] {
        let body: [String: Any] = ["m_customer_id": mCustomerId, "order_id": orderId]
        let endpoint = APIEndpoint.customerCards(body: body)
        let json = try await networkClient.requestRawJSON(endpoint)
        guard let rows = json["data"] as? [[String: Any]] else { return [] }
        return rows.map { CardData.fromSDK($0) }
    }
