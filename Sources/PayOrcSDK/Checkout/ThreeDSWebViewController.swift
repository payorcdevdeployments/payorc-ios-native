...
public final class ThreeDSWebViewController: UIViewController {
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        // This screen draws its own top bar (below) — hide the wrapping
        // UINavigationController's bar so it doesn't stack on top of it and inflate
        // the header height / intercept taps meant for our close button.
    ...
    private func setupWebView() {
        ...
        // Installed as a WKUserScript at .atDocumentStart (not injected reactively
        // from didFinish) so the console.log/postMessage hooks exist BEFORE any of
        // the page's own scripts run. Some PayOrc postback pages emit their
        // `postback message {...}` synchronously during initial page load — with
        // evaluateJavaScript() called from didFinish, that console.log had already
        // fired before our hook was installed, so it was silently missed even
        // though the navigation itself completed fine. A WKUserScript re-injects
        // automatically on every navigation in the webview, including every
        // redirect hop in the 3DS chain, not just the first page.
    ...
    /// Mirrors Flutter's `PopScope` handler on `AddCardPaymentWebView`: closing the
    /// 3DS challenge without a terminal postback must still deliver a terminal
    /// result to the host — `PayorcCancellation.payload()` — so the caller never
    /// hangs waiting for `onCompleted`.
    ///
    /// Does not dismiss itself — the presenter up the chain (``SubmittingOrderViewController``
    /// / ``CheckoutViewController``) owns dismissal so the whole presented stack collapses
    /// in a single animation instead of revealing intermediate sheets one at a time.
    ...
    private func handleRawMessage(_ raw: String) {
        ...
        // A real 3DS2 challenge fires many messages that have nothing to do with
        // PayOrc's own postback (Cardinal Commerce's setup JWT, `profile.completed`,
        // iframe-sizing pings, etc.) — mostly via `window.postMessage`, which our
        // injected bridge forwards unfiltered (unlike console.log, which the JS side
        // already filters by pattern). Every genuine PayOrc `sdk/payment` postback —
        // success or failure — includes a `p_order_id` key, so this mirrors Flutter's
        // `_handleBridgeMessage` guard: drop anything else before it ever reaches the
        // JSON/regex completion checks below.
        guard raw.contains("p_order_id") else {
            #if DEBUG
            let preview = raw.count > 200 ? String(raw.prefix(200)) + "…" : raw
            print("[PayOrc 3DS] Ignored non-postback bridge message: \(preview)")
            #endif
            return
        }
    ...
    /// Fires `onCompleted` with the terminal postback. Does not dismiss itself —
    /// see ``closeTapped()`` for why dismissal is left to the presenter up the chain.
    private func finish(with body: [String: Any]) {
        guard !completed else { return }
        completed = true

        #if DEBUG
        print("[PayOrc 3DS] Terminal postback received.")
        #endif
...
extension ThreeDSWebViewController: WKNavigationDelegate {
    public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        #if DEBUG
        print("[PayOrc 3DS] didFinish navigation → \(webView.url?.absoluteString ?? "nil")")
        #endif
        injectBridge()
    ...
    public func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
        #if DEBUG
        print("[PayOrc 3DS] didStartProvisionalNavigation → \(webView.url?.absoluteString ?? "nil")")
        #endif
    }

    public func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        // Previously unhandled — a failure here (blocked request, cert/ATS issue,
        // timeout) left the webview stuck on whatever it last rendered with zero
        // console output, which looks identical to "stuck loading forever".
        #if DEBUG
        print("[PayOrc 3DS] ❌ didFailProvisionalNavigation for \(webView.url?.absoluteString ?? "nil"): \(error)")
        #endif
    }

    public func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        #if DEBUG
        print("[PayOrc 3DS] ❌ didFail for \(webView.url?.absoluteString ?? "nil"): \(error)")
        #endif
    }
    ...
    public func webView(
        ...
        #if DEBUG
        print("[PayOrc 3DS] decidePolicyFor \(navigationAction.navigationType.rawValue) → \(navigationAction.request.url?.absoluteString ?? "nil")")
        #endif
...
extension ThreeDSWebViewController: WKUIDelegate {
    /// Some 3DS/ACS challenge pages call `window.open()` (or a link with
    /// `target="_blank"`) instead of a normal same-frame redirect. Without a
    /// `WKUIDelegate`, WKWebView just silently drops that request — no error, no
    /// navigation delegate callback, nothing — which looks identical to the page
    /// being stuck. Load it in this same webview instead of trying to spawn a
    /// new window we don't have one to give it.
    public func webView(
        _ webView: WKWebView,
        ...
        #if DEBUG
        print("[PayOrc 3DS] window.open intercepted → \(navigationAction.request.url?.absoluteString ?? "nil")")
        #endif
...
// Shared with ``AddCardVerificationWebViewController`` — same postback-forwarding
// bridge, different Swift-side completion logic per webview.
func payOrcBridgeJS(channelName: String) -> String {
    """
    (function () {
