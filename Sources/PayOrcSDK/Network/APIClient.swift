...
public final class NetworkClient {
    ...
    /// Sends a signed request and returns the decoded JSON object directly, for
    /// endpoints whose response shape doesn't fit a fixed `Decodable` (e.g.
    /// `sdk/customer/cards`, where `data` is an array of rows with dynamic keys).
    ///
    /// - Throws: ``PayOrcError`` on any failure (network, HTTP, non-object JSON).
    public func requestRawJSON(_ endpoint: APIEndpoint) async throws -> [String: Any] {
        let urlRequest = try endpoint.buildRequest(configuration: configuration)
        return try await performRawWithRetry(urlRequest: urlRequest)
    }

    private func performRawWithRetry(
        ...
        do {
            ...
            guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
                throw PayOrcError.decodingFailed("Response was not a JSON object.")
            }
            return json
            ...
            if error.isRetryable && attempt < Self.maxRetryCount {
                ...
                return try await performRawWithRetry(urlRequest: urlRequest, attempt: attempt + 1)
            }
            throw error
