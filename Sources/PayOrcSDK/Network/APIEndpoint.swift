...
public enum APIEndpoint {
    ...
    /// Uses the environment-resolved host (dev-gateway for sandbox, production
    /// gateway for production), same as payment / add-card / etc.
    ...
    /// Accepts pre-built ordered JSON `Data` (Flutter-compatible byte order, no \/
    /// escaping) — same requirement as `.payment`: the gateway validates the HMAC
    /// over Dart's exact `jsonEncode` insertion order, not `.sortedKeys` output.
    case addCard(bodyData: Data)
...
extension APIEndpoint {
    ...
    func buildRequest(configuration: PayOrcConfiguration) throws -> URLRequest {
        ...
        } else if case .addCard(let prebuiltData) = self {
            // Same reasoning as .payment above — sdk/add-card shares the same
            // order-sensitive signature validation (confirmed via E0021 with the
            // sorted-keys path).
            bodyData = prebuiltData
        } else {
