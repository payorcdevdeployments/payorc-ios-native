...
public final class PayOrc {
    ...
    // MARK: - Payment Options (Bottom Sheet) + Failure Recovery

    /// Presents the full payment-options bottom sheet (Apple Pay / Tabby / Pay with
    /// Card) and owns the whole flow through to a terminal `delegate` callback —
    /// mirrors Flutter's `PayorcSdk.showOptionsSheet`.
    ///
    /// The only difference from wiring ``PaymentOptionsViewController`` yourself (as
    /// ``presentCheckout(from:request:delegate:)`` and
    /// `PayOrcEmbeddedPayWithCardButton` do): when a card payment started from this
    /// sheet fails, instead of an inline error on the card form, this presents
    /// ``PaymentFailedViewController`` — showing the error, the customer's saved
    /// cards (fetched from `sdk/customer/cards`), and other payment methods to retry
    /// with. That recovery sheet is specific to this entry point; it does not apply
    /// to ``presentCheckout(from:request:delegate:)`` or the embedded card button.
    ///
    /// - Parameters:
    ///   - viewController: The presenting `UIViewController`.
    ///   - request:        The ``PaymentRequest`` describing the order.
    ///   - delegate:       Receives ``PayOrcSDKDelegate`` callbacks for the flow's outcome.
    @MainActor
    public func presentPaymentOptions(
        ...
        let options = PaymentOptionsViewController(paymentRequest: request)

        options.onSelectCard = { [weak self, weak viewController] in
            guard let self, let viewController else { return }
            self.presentCardCheckoutWithFailureRecovery(
                from: viewController, request: request, delegate: delegate
            )
        }
        options.onSelectApplePay = { [weak self] in
            guard let self else { return }
            self.showApplePayCheckout(
                request: request,
                onAuthorized: { [weak delegate] result in delegate?.payOrcDidComplete(result: result) },
                onError:      { [weak delegate] error  in delegate?.payOrcDidFail(error: error) }
            )
        }
        options.onSelectTabby = { [weak self, weak viewController] in
            guard let self, let viewController else { return }
            self.showTabbyCheckout(
                from: viewController,
                request: request,
                onAuthorized: { [weak delegate] result in delegate?.payOrcDidComplete(result: result) },
                onError:      { [weak delegate] error  in delegate?.payOrcDidFail(error: error) }
        ...
        options.onCancel = { [weak delegate] in delegate?.payOrcDidCancel() }

        viewController.present(options, animated: true)
    }

    /// Presents the card-entry form with ``CheckoutViewController/onSubmitFailed``
    /// wired to ``presentPaymentFailedSheet(from:failedCard:error:request:delegate:)``.
    @MainActor
    private func presentCardCheckoutWithFailureRecovery(
        from viewController: UIViewController,
        request:  PaymentRequest,
        delegate: PayOrcSDKDelegate
    ) {
        let viewModel = CheckoutViewModel(paymentRequest: request, paymentRepository: paymentRepository)
        let checkoutVC = CheckoutViewController(viewModel: viewModel, delegate: delegate)
        checkoutVC.onSubmitFailed = { [weak self, weak viewController] error, card, orderId in
            guard let self, let viewController else {
                #if DEBUG
                print("[PayOrc] onSubmitFailed fired but self or viewController was already deallocated — dropped.")
                #endif
                return
            }
            #if DEBUG
            print("[PayOrc] onSubmitFailed → presenting PaymentFailedViewController. orderId=\(orderId ?? "nil")")
            #endif
            self.presentPaymentFailedSheet(
                from: viewController, failedCard: card, error: error, resolvedOrderId: orderId,
    ...
    /// Presents ``PaymentFailedViewController`` for a failed card payment, wiring its
    /// retry / add-new-card / other-method callbacks back into the same flow.
    @MainActor
    private func presentPaymentFailedSheet(
        from viewController: UIViewController,
        failedCard: CardData,
        error: PayOrcError,
        ...
        let methods = checkoutCustomization?.availableMethods ?? []
        let applePayEnabled = methods.contains { $0.isVisible && $0.type.lowercased() == "apple_pay" }
        let tabbyEnabled = methods.contains { $0.isVisible && $0.type.lowercased() == "tabby" }
            && !(configuration.tabbyApiKey ?? "").trimmingCharacters(in: .whitespaces).isEmpty

        let errorCode: String
        if case .paymentFailed(let code, _) = error { errorCode = code }
        else if case .apiFailure(let code, _) = error { errorCode = code }
        else { errorCode = "" }

        let failedVC = PaymentFailedViewController(
        ...
        failedVC.onRetryOriginalCard = { [weak self, weak viewController] in
            guard let self, let viewController else { return }
            self.presentDirectSubmission(
                from: viewController, card: failedCard, request: request,
                delegate: delegate, isSavedCardRetry: false
            )
        }
        failedVC.onRetrySavedCard = { [weak self, weak viewController] savedCard, cvv in
            guard let self, let viewController else { return }
            let retryCard = CardData(
                ...
                cardNetwork: savedCard.cardNetwork
            )
            self.presentDirectSubmission(
                from: viewController, card: retryCard, request: request,
                delegate: delegate, isSavedCardRetry: true
            )
        }
        failedVC.onAddNewCard = { [weak self, weak viewController] in
            guard let self, let viewController else { return }
            self.presentCardCheckoutWithFailureRecovery(from: viewController, request: request, delegate: delegate)
        }
        failedVC.onApplePay = { [weak self] in
            guard let self else { return }
            self.showApplePayCheckout(
                request: request,
                onAuthorized: { [weak delegate] result in delegate?.payOrcDidComplete(result: result) },
        ...
        failedVC.onTabby = { [weak self, weak viewController] in
            guard let self, let viewController else { return }
            self.showTabbyCheckout(
                from: viewController,
                request: request,
        ...
        failedVC.onCancel = { [weak delegate] in delegate?.payOrcDidCancel() }

        viewController.present(failedVC, animated: true)
    }

    /// Submits `card` directly (no card-entry form) via ``SubmittingOrderViewController``,
    /// presented straight from `viewController`. Used for "Retry Payment" (same card)
    /// and saved-card retries from ``PaymentFailedViewController``.
    @MainActor
    private func presentDirectSubmission(
        from viewController: UIViewController,
        card: CardData,
        request: PaymentRequest,
        delegate: PayOrcSDKDelegate,
        isSavedCardRetry: Bool
    ) {
        let viewModel = CheckoutViewModel(paymentRequest: request, paymentRepository: paymentRepository)
        let submittingVC = SubmittingOrderViewController(
            viewModel: viewModel,
            card: card,
            useTokenSubmission: isSavedCardRetry,
            onSuccess: { [weak viewController] result in
                guard let viewController else { return }
                // Unlike the CheckoutViewController flow, nothing has dismissed itself
                // yet on this path (no self-dismiss on success — see
                // SubmittingOrderViewController), so this dismiss is what collapses the
                // submitting sheet (and 3DS webview, if shown) in one animation.
                viewController.dismiss(animated: true) {
                    let confirmedVC = PaymentConfirmedViewController(card: card) {
                        delegate.payOrcDidComplete(result: result)
                    }
                    viewController.present(confirmedVC, animated: true)
                }
            },
            onFailure: { [weak self, weak viewController] error, orderId in
                guard let self, let viewController else { return }
                // Nothing has dismissed itself yet (no self-dismiss on failure — see
                // SubmittingOrderViewController), so this dismiss is what collapses the
                // submitting sheet (and 3DS webview, if shown) in one animation before
                // presenting the failure sheet fresh from viewController.
                viewController.dismiss(animated: true) {
                    self.presentPaymentFailedSheet(
                        from: viewController, failedCard: card, error: error, resolvedOrderId: orderId,
                        request: request, delegate: delegate
...
private extension PayOrc {
    ...
    static func applyHostOverrides(_ c: PayOrcSDKCustomization) {
        ...
        // Input border style / guidance style / autoselect color
        if let v = c.inputBorderStyle { PayOrcUIConstants._hostInputBorderStyle = v }
        if let v = c.guidanceStyle    { PayOrcUIConstants._hostGuidanceStyle    = v }
        if let v = c.autoselectColor  { PayOrcUIConstants._hostAutoselectColor  = v }

        // Add-card form copy overrides — merge with any previous call, like Flutter's
        // PayorcSdkAddCardFormCustomization.merge.
        if let form = c.addCardForm {
            PayOrcUIConstants._hostAddCardForm = PayOrcAddCardFormCustomization.merge(
                PayOrcUIConstants._hostAddCardForm, form
            )
        }

        // Card form validation message overrides
        if let v = c.cardFormValidation { PayOrcUIConstants._hostCardFormValidation = v }
        ...
        if let t = c.text {
            ...
            if let v = t.fontWeight { PayOrcUIConstants._hostTextFontWeight = v }
            if let v = t.letterSpacing  { PayOrcUIConstants._hostTextLetterSpacing = v }
            if let v = t.textAlignment  { PayOrcUIConstants._hostTextAlignment     = v }
            if let v = t.maxLines       { PayOrcUIConstants._hostTextMaxLines      = v }
            if let v = t.lineBreakMode  { PayOrcUIConstants._hostTextLineBreakMode = v }
            if let v = t.underline      { PayOrcUIConstants._hostTextUnderline     = v }
            if let v = t.strikethrough  { PayOrcUIConstants._hostTextStrikethrough = v }
            if let v = t.wordSpacing    { PayOrcUIConstants._hostTextWordSpacing   = v }
            if let v = t.italic         { PayOrcUIConstants._hostTextItalic        = v }
            if let v = t.shadow         { PayOrcUIConstants._hostTextShadow        = v }
        }
