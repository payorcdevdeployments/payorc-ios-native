...
final class PaymentMethodTileView: UIView {
    ...
    func configure(with config: Config) {
        // ── Colors ────────────────────────────────────────────────────────────
        applyColors(isSelected: config.isSelected, customBg: config.backgroundColor, customTitle: config.titleColor)
        ...
        if !config.trailingLogos.isEmpty {
            for name in config.trailingLogos {
    ...
    private func applyColors(isSelected: Bool, customBg: UIColor? = nil, customTitle: UIColor? = nil) {
        ...
        if isSelected {
            ...
            backgroundColor = baseBg.darkened(by: 0.1)
